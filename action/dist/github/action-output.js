...
async function appendOutputRecord(path, record) {
    await appendFile(path, record);
}
async function appendSummary(path, summary) {
    await appendFile(path, summary, "utf8");
}
export function serializeGitHubIntegrationResult(result) {
    return canonicalV1Json(result);
}
async function writeGitHubActionOutputRecord(result, options, terminatePartialLine) {
    ...
    const record = Buffer.from(`${terminatePartialLine ? "\n" : ""}${ET_STATUS_OUTPUT_NAME}=${result.status}\n${ET_RESULT_JSON_OUTPUT_NAME}=${etResultJson}\n`, "utf8");
    const maxOutputRecordBytes = options.maxOutputRecordBytes ?? MAX_ET_ACTION_OUTPUT_UTF8_BYTES;
...
async function publishIntegrationError(options, terminatePartialOutputLine) {
    const failure = await integrationErrorResult();
    try {
        await writeGitHubActionOutputRecord(failure, options, terminatePartialOutputLine);
    }
    catch {
...
export async function publishGitHubActionResult(result, options) {
    try {
