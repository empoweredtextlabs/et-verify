...
function parseDeclaration(bytes) {
    let parsed;
    try {
        ...
        return {
            declaration: null,
            issue: issue("acceptance_declaration_json_invalid", "The committed acceptance declaration was not valid UTF-8 JSON."),
        };
    }
    if (!isRecord(parsed) ||
        ...
        return {
            declaration: null,
            issue: issue("acceptance_declaration_schema_invalid", "The committed acceptance declaration did not satisfy the bounded V1 schema."),
        };
    }
    const normalizedPaths = normalizePathSet(parsed.changeScope.paths, "scope_claim");
    if (normalizedPaths.limitations.length > 0 ||
        normalizedPaths.paths.length !== parsed.changeScope.paths.length) {
        return {
            declaration: null,
            issue: issue("acceptance_declaration_paths_invalid", "The complete changed-path declaration contained an invalid or duplicate normalized repository-relative path."),
        };
    }
    const checks = [];
    ...
    for (const candidate of parsed.checkResults) {
        if (!isRecord(candidate) ||
            ...
            return {
                declaration: null,
                issue: issue("acceptance_declaration_checks_invalid", "The declaration's required TEST/BUILD PASS checks were absent, duplicated, malformed, or unsupported."),
            };
        }
...
function parseTreeEntry(output) {
    ...
    const match = text.match(/^(100644|100755) blob ([0-9a-f]{40}(?:[0-9a-f]{24})?)\t\.et-verify\/acceptance\.json$/);
    return match === null
...
export async function loadV1AcceptanceDeclarationFromHead(input) {
    ...
    if (!baseTree.ok || !headTree.ok) {
        return {
            ...
            issues: [
                issue("acceptance_declaration_git_read_failed", "The acceptance declaration could not be read safely from the evaluated Git commits."),
            ],
        };
    }
    ...
    if (base.state === "MALFORMED" || head.state === "MALFORMED") {
        return {
            ...
            changedFromBase: null,
            issues: [
                issue("acceptance_declaration_tree_entry_invalid", "The acceptance declaration tree entry was not a regular Git blob with the required repository-relative path."),
            ],
        };
    }
    if (head.state === "MISSING" || head.objectId === null) {
        return {
            ...
            issues: [
                issue("acceptance_declaration_missing", "Required ET acceptance declaration was not present and state-bound; add or repair it and rerun."),
            ],
        };
    }
    ...
    if (!blob.ok || blob.stdout.byteLength > MAX_DECLARATION_BYTES) {
        return {
            ...
            issues: [
                issue("acceptance_declaration_blob_unavailable", "The committed acceptance declaration blob could not be read within the bounded V1 size limit."),
            ],
        };
    }
...
//# sourceMappingURL=load-acceptance-declaration.js.map