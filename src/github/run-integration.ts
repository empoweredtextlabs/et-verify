import type { GitReadonlyRunner } from "../git/run-git-readonly.js";
import {
  evaluatePreparedV1Acceptance,
  prepareV1Acceptance,
  type EvaluatePreparedV1AcceptanceOptions,
  type V1PreparedAcceptanceState,
} from "../v1/evaluate-acceptance.js";
import type { V1AcceptanceResult } from "../v1/types.js";
import {
  executeTrustedChecks,
  type TrustedCheckProcessExecutor,
} from "./execute-trusted-checks.js";
...
import {
  ...
  RUNTIME_AUTHORITY_TOPOLOGY,
  V1_TRUSTED_CHECK_CONFIG_PATH,
  type GitHubIntegrationInput,
  type GitHubIntegrationResult,
} from "./types.js";

export interface GitHubIntegrationOverrides {
  gitRunner?: GitReadonlyRunner;
  checkProcessExecutor?: TrustedCheckProcessExecutor;
  prepareAcceptance?: (input: {
    repositoryPath: string;
    baseRef: string;
    headRef: string;
  }) => Promise<V1PreparedAcceptanceState>;
  evaluatePreparedAcceptance?: (
    options: EvaluatePreparedV1AcceptanceOptions,
  ) => V1AcceptanceResult;
}

export function githubExitCode(
  status: GitHubIntegrationResult["status"],
): 0 | 1 {
...
function reviewResult(input: {
  reasonCode: string;
  reason: string;
  identity?: GitHubIntegrationResult["identity"];
  configSha256?: string;
...
export async function runGitHubPullRequestIntegration(
  input: GitHubIntegrationInput,
  overrides: GitHubIntegrationOverrides = {},
  ...
  if (
    ...
    return reviewResult({
      ...
      reason:
        "The exact evaluated Git state could not be captured before configured checks executed.",
      identity,
  ...
  if (capture.workingTree.clean !== true) {
    return reviewResult({
      ...
      reason:
        "The checked-out PR head contained mutable working-tree state before trusted checks executed.",
      identity,
      configSha256: config.configSha256,
    });
  }

  const trustBaseChanged = capture.changedPaths.some(
    (path) =>
      path === V1_TRUSTED_CHECK_CONFIG_PATH ||
  ...
  const integrationReviewIssues = checks.flatMap((check) =>
  ...
  const evaluate =
    overrides.evaluatePreparedAcceptance ?? evaluatePreparedV1Acceptance;
  const evaluatorResult = evaluate({
    preparedState,
    trustedConfiguredChecks: config.configuration.checks.map(
      ({ checkId, kind }) => ({ checkId, kind }),
    ),
    trustedCheckEvidence: checks.flatMap((check) =>
  ...
  const reasonCode =
    evaluatorResult.status === "ACCEPTED"
      ? "ACCEPTED"
...
export async function runGitHubActionBoundary(
  run: () => Promise<GitHubIntegrationResult>,
): Promise<GitHubIntegrationResult> {
  try {
    return await run();
  } catch {
    return reviewResult({
      ...
      reason:
        "The trusted ET integration encountered an unexpected error; the GitHub result failed closed.",
    });
  }
}
