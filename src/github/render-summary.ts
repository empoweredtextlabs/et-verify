...
function displayStatus(status: GitHubIntegrationResult["status"]): string {
...
export function renderGitHubSummary(result: GitHubIntegrationResult): string {
  const evaluator = result.evaluatorResult;
  const claimedChecks =
    evaluator?.agentClaimed.declaration?.checkResults
  ...
  const requiredAction =
    evaluator?.requiredHumanAction.join(" ") ??
  ...
  const checks =
    result.checks.length === 0
      ...
      : result.checks
          .map(
            (check) =>
              `- ${check.kind} ${code(check.checkId)}: ${check.state}${
                check.indeterminateReason === null
                  ? ""
