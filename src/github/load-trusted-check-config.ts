...
import type {
  TrustedConfigLoadResult,
  V1TrustedCheckConfiguration,
  V1TrustedCheckDefinition,
} from "./types.js";
...
function parseTreeEntry(bytes: Buffer): string | null | "MISSING" {
  ...
  const match = text.match(
    /^(100644|100755) blob ([0-9a-f]{40}(?:[0-9a-f]{24})?)\t\.et-verify\/checks\.json$/,
  );
  return match?.[2] ?? null;
}
...
function parseConfiguration(bytes: Buffer): V1TrustedCheckConfiguration | null {
  let parsed: unknown;
  try {
  ...
  const checks: V1TrustedCheckDefinition[] = [];
  for (const candidate of parsed.checks) {
    if (
      ...
      (candidate.timeoutMs as number) < MIN_TIMEOUT_MS ||
      (candidate.timeoutMs as number) > MAX_TIMEOUT_MS
    ) {
      return null;
    }
...
export async function loadTrustedCheckConfiguration(input: {
...
}): Promise<TrustedConfigLoadResult> {
  ...
  if (!tree.ok) {
    return {
      ...
      reason:
        "Trusted check configuration could not be read from the evaluated base commit.",
    };
  }
  ...
  if (objectId === "MISSING") {
    return {
      ...
      reasonCode: "TRUSTED_CHECK_CONFIG_UNAVAILABLE_AT_EVALUATED_BASE",
      reason:
        "No trusted check configuration exists at the evaluated base; bootstrap and pre-configuration PRs require review.",
    };
  }
