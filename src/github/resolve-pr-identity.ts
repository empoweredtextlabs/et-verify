...
function eventCommit(payload: unknown, side: "base" | "head"): string | null {
...
function decodeCommitSha(bytes: Buffer): string | null {
  try {
...
function failure(
  reasonCode: Exclude<GitHubIdentityResolution, { ok: true }>["reasonCode"],
  reason: string,
): GitHubIdentityResolution {
...
export async function resolveGitHubPullRequestIdentity(input: {
  ...
  if (input.eventName !== "pull_request") {
    return failure(
      ...
      "V1 alpha supports pull_request only; merge_group and all other events fail closed.",
    );
  }
  ...
  if (githubBaseTipSha === null || githubHeadSha === null) {
    return failure(
      ...
      "The pull request event did not contain canonical full base-tip and head commit SHAs.",
    );
  }
  if (input.githubSha !== undefined && input.githubSha === githubHeadSha) {
    return failure(
      "SYNTHETIC_MERGE_HEAD_REJECTED",
      "The evaluated head must come from pull_request.head.sha and must not be the GitHub merge-ref identity.",
    );
  }
  ...
  if (decodeCommitSha(checkedOut.stdout) !== githubHeadSha) {
    return failure(
      ...
      "The checked-out HEAD did not equal the exact pull_request.head.sha.",
    );
  }
  ...
  if (!mergeBase.ok || evaluatedBaseSha === null) {
    return failure(
      ...
      "The PR merge-base could not be established from complete local history; no base-tip fallback was used.",
    );
  }
  const identity: GitHubPullRequestIdentity = {
    githubBaseTipSha,
    githubHeadSha,
    evaluatedBaseSha,
