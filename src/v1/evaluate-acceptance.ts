...
import type { Cf003ActualChangeSet, Cf003SemanticFinding } from "../cf003/types.js";
import { hashPortableGitEvidence } from "../git/canonicalize-git-evidence.js";
import { captureGitState } from "../git/capture-git-state.js";
import type { GitStateCaptureResult } from "../git/types.js";
import { canonicalV1Json, hashV1Value } from "./canonicalize.js";
import {
  loadV1AcceptanceDeclarationFromHead,
  type V1DeclarationLoadResult,
} from "./load-acceptance-declaration.js";
import {
  evaluateTrustedCheckEvidence,
  loadV1CheckEvidence,
  type V1LoadedEvidence,
} from "./load-check-evidence.js";
import type {
  EvaluateV1AcceptanceOptions,
  V1AcceptanceDeclaration,
  V1AcceptanceIssue,
  V1AcceptanceResult,
  V1AcceptanceStatus,
  V1CheckEvaluation,
  V1IntegrationReviewIssue,
  V1StateBinding,
  V1TrustedActionCheckEvidence,
  V1TrustedConfiguredCheckIdentity,
  V1TrustBaseReview,
} from "./types.js";
import {
  V1_ACCEPTANCE_CONTRACT_VERSION,
  V1_ACCEPTANCE_DECLARATION_PATH,
} from "./types.js";

const SCOPE_CLAIM_TEXT =
  "The committed V1 acceptance declaration lists the complete final changed-path set.";

export interface V1PreparedAcceptanceState {
  capture: GitStateCaptureResult;
  declarationLoad: V1DeclarationLoadResult;
  loadedExternalEvidence: V1LoadedEvidence[];
}

export interface EvaluatePreparedV1AcceptanceOptions {
  preparedState: V1PreparedAcceptanceState;
  trustedConfiguredChecks: readonly V1TrustedConfiguredCheckIdentity[];
  trustedCheckEvidence?: readonly V1TrustedActionCheckEvidence[];
  trustBaseReview?: V1TrustBaseReview;
  integrationReviewIssues?: readonly V1IntegrationReviewIssue[];
}

function issue(
  code: string,
  status: V1AcceptanceIssue["status"],
  summary: string,
): V1AcceptanceIssue {
  return { code, status, summary };
}

function unavailableDeclarationLoad(): V1DeclarationLoadResult {
  return {
    state: "UNAVAILABLE",
    declaration: null,
    ...
    issues: [
      issue(
        "acceptance_declaration_state_unavailable",
        "REVIEW_REQUIRED",
        "The declaration could not be read from HEAD because the evaluated Git state was unavailable.",
      ),
    ],
  };
}

function uniqueIssues(issues: readonly V1AcceptanceIssue[]): V1AcceptanceIssue[] {
  const unique = new Map<string, V1AcceptanceIssue>();
  for (const candidate of issues) {
    unique.set(canonicalV1Json(candidate), candidate);
  }
  return [...unique.values()].sort(
    (left, right) =>
      (left.status === right.status ? 0 : left.status === "BLOCKED" ? -1 : 1) ||
      compareCanonicalStrings(left.code, right.code) ||
      compareCanonicalStrings(left.summary, right.summary),
  );
}

function portableChangeSet(capture: GitStateCaptureResult): Cf003ActualChangeSet {
...
function scopeFinding(
  capture: GitStateCaptureResult,
  declaration: V1AcceptanceDeclaration,
  declarationIdentitySha256: string,
): Cf003SemanticFinding {
...
function evaluateChecks(
  ...
  trustedConfiguredChecks: readonly V1TrustedConfiguredCheckIdentity[],
  loadedEvidence: readonly V1LoadedEvidence[],
): { checks: V1CheckEvaluation[]; issues: V1AcceptanceIssue[] } {
  const issues: V1AcceptanceIssue[] = [];
  const declaredChecks = declaration?.checkResults ?? [];
  const configuredChecks = [...trustedConfiguredChecks].sort(
    (left, right) =>
      compareCanonicalStrings(left.checkId, right.checkId) ||
      compareCanonicalStrings(left.kind, right.kind),
  );
  const configuredById = new Map(
    configuredChecks.map((configured) => [configured.checkId, configured]),
  );
  const declaredById = new Map(
    declaredChecks.map((declared) => [declared.checkId, declared]),
  );
  const evaluations = new Map<string, V1CheckEvaluation>();
  ...
  for (const declared of declaredChecks) {
    ...
    if (configured === undefined) {
      issues.push(
        issue(
          `declared_check_not_configured:${declared.checkId}`,
          "REVIEW_REQUIRED",
          `Declared ${declared.kind} check ${declared.checkId} is not present in the trusted evaluated-base configuration.`,
        ),
      );
      ...
      issues.push(
        issue(
          `declared_check_kind_mismatch:${declared.checkId}`,
          "REVIEW_REQUIRED",
          `Declared check ${declared.checkId} uses kind ${declared.kind}, but the trusted evaluated-base configuration requires ${configured.kind}.`,
        ),
      );
      evaluations.set(declared.checkId, {
        ...declared,
  ...
  for (const configured of configuredChecks) {
    ...
    const matching = loadedEvidence.filter(
      (loaded) =>
        loaded.evidence?.checkId === configured.checkId &&
        loaded.evidence.kind === configured.kind,
    );
    const failures = matching.filter(
      (loaded) => loaded.observation.state === "QUALIFYING_FAIL",
    );
    const passes = matching.filter(
      (loaded) => loaded.observation.state === "QUALIFYING_PASS",
    );
    const qualifyingEvidenceIdentities = [...failures, ...passes]
      .flatMap((loaded) =>
        loaded.observation.evidenceIdentitySha256 === null
          ? []
    ...
    if (failures.length > 0) {
      issues.push(
        issue(
          `required_check_failed:${configured.checkId}`,
          "BLOCKED",
          `Trusted ${configured.kind} evidence established that required configured check ${configured.checkId} failed.`,
        ),
      );
    ...
    if (declared === undefined) {
      issues.push(
        issue(
          `required_check_not_declared:${configured.checkId}`,
          "REVIEW_REQUIRED",
          `Required configured ${configured.kind} check ${configured.checkId} was not declared.`,
        ),
      );
      continue;
    }
    ...
    issues.push(
      issue(
        `required_check_evidence_missing:${declared.checkId}`,
        "REVIEW_REQUIRED",
        `Qualifying state-bound trusted evidence was absent for required ${declared.kind} check ${declared.checkId}.`,
      ),
    );
    evaluations.set(declared.checkId, {
      ...declared,
  ...
  const checks = declaredChecks.map(
    (declared): V1CheckEvaluation =>
      evaluations.get(declared.checkId) ?? {
        ...declared,
        establishedState: "INDETERMINATE",
        qualifyingEvidenceIdentities: [],
      },
...
function statusFor(issues: readonly V1AcceptanceIssue[]): V1AcceptanceStatus {
...
function issueCheckIds(
  issues: readonly V1AcceptanceIssue[],
  prefix: string,
): string[] {
  return issues
    .flatMap((candidate) =>
...
function checkList(checkIds: readonly string[]): string {
...
function requiredAction(
  ...
  if (failed.length > 0) {
    actions.push(
      `Repair required configured check(s) ${checkList(failed)}, produce fresh qualifying PASS evidence, ensure the declaration covers the complete configured check set, and rerun ET.`,
    );
  }
  if (omitted.length > 0) {
    actions.push(
      `Declare required configured check(s) ${checkList(omitted)} with their configured kind and rerun ET.`,
    );
  }
  if (extra.length > 0) {
    actions.push(
      `Remove declaration entries for unconfigured check(s) ${checkList(extra)} and rerun ET.`,
    );
  }
  if (mismatched.length > 0) {
    actions.push(
      `Correct the declared kind for configured check(s) ${checkList(mismatched)} and rerun ET.`,
    );
  }
  if (actions.length > 0) {
    return actions;
...
export function serializeV1AcceptanceResult(result: V1AcceptanceResult): string {
...
export async function prepareV1Acceptance(input: {
...
}): Promise<V1PreparedAcceptanceState> {
  ...
  const declarationLoad =
    resolvedRepositoryPath !== null && baseCommit !== null && headCommit !== null
  ...
  const loadedExternalEvidence =
    resolvedRepositoryPath !== null && headCommit !== null
 ...
 */
export function evaluatePreparedV1Acceptance(
  options: EvaluatePreparedV1AcceptanceOptions,
): V1AcceptanceResult {
  ...
  if (capture.captureStatus !== "COMPLETE") {
    issues.push(
      issue(
        "evaluated_git_state_incomplete",
        "REVIEW_REQUIRED",
        "The exact evaluated base, head, changed paths, and final net patch could not be captured completely.",
      ),
    );
  }
  ...
  const declarationIdentitySha256 =
    declarationLoad.headBlobGitObjectId === null ||
  ...
  if (
    ...
    issues.push(
      issue(
        "acceptance_declaration_base_mismatch",
        "REVIEW_REQUIRED",
        "The declared base commit did not match the exact evaluated base commit.",
      ),
    );
  }
  if (declaration !== null && declarationLoad.changedFromBase !== true) {
    issues.push(
      issue(
        "acceptance_declaration_not_changed",
        "REVIEW_REQUIRED",
        "The acceptance declaration was not changed from the evaluated base and therefore was not fresh for this state.",
      ),
    );
  }

  const declarationIncludesItself =
    declaration?.changeScope.paths.includes(V1_ACCEPTANCE_DECLARATION_PATH) ?? false;
  const actualIncludesDeclaration = capture.changedPaths.includes(
    V1_ACCEPTANCE_DECLARATION_PATH,
  );
  const declarationFreshEnoughForScopeComparison =
    declaration !== null &&
    baseCommit !== null &&
    declaration.baseCommit === baseCommit &&
  ...
  if (declaration !== null && !declarationIncludesItself) {
    issues.push(
      issue(
        "acceptance_declaration_self_inclusion_violation",
        declarationFreshEnoughForScopeComparison
          ? "BLOCKED"
          : "REVIEW_REQUIRED",
        "The V1 declaration must include .et-verify/acceptance.json in its own complete changed-path set.",
      ),
    );
  }
  if (declaration !== null && !actualIncludesDeclaration) {
    issues.push(
      issue(
        "acceptance_declaration_not_in_final_patch",
        "REVIEW_REQUIRED",
        "The declaration path was not changed in the evaluated final net patch, so declaration freshness was not established.",
      ),
    );
  }

  let finding: Cf003SemanticFinding | null = null;
  let scopeState: V1AcceptanceResult["etEstablished"]["scopeState"] =
    "INDETERMINATE";
  if (
    declaration !== null &&
    ...
    if (scopeState === "MISMATCH") {
      issues.push(
        issue(
          ...
          "BLOCKED",
          "The committed complete changed-path declaration contradicted the exact final changed-path set.",
        ),
      );
    } else if (scopeState === "INDETERMINATE") {
      issues.push(
        issue(
          "acceptance_exact_path_set_indeterminate",
          "REVIEW_REQUIRED",
          "The complete changed-path declaration could not be compared deterministically.",
        ),
      );
    }
  }
  ...
  const trustedConfiguredChecksAvailable =
    options.trustedConfiguredChecks.length > 0;
  const checkEvaluation = !trustedConfiguredChecksAvailable
    ? {
        checks: (declaration?.checkResults ?? []).map(
          (declared): V1CheckEvaluation => ({
            ...declared,
            establishedState: "INDETERMINATE",
            qualifyingEvidenceIdentities: [],
    ...
    : evaluateChecks(
        declaration,
        options.trustedConfiguredChecks,
        loadedEvidence,
      );
  ...
  if (!trustedConfiguredChecksAvailable) {
    issues.push(
      issue(
        "trusted_check_configuration_unavailable",
        "REVIEW_REQUIRED",
        "The authoritative trusted configured check set was unavailable, so acceptance could not be established.",
      ),
    );
  }
  ...
  if (trustBaseReviewRequired) {
    ...
    issues.push(
      issue(
        `trust_base_review_required:${reasonCode}`,
        "REVIEW_REQUIRED",
        "The evidence-producing trust base changed and policy requires human review.",
      ),
    );
  }
  for (const review of options.integrationReviewIssues ?? []) {
    ...
    issues.push(
      issue(
        `integration_review_required:${reasonCode}`,
        "REVIEW_REQUIRED",
        summary,
      ),
    );
  ...
  const stateBinding: V1StateBinding = {
  ...
  const bindingState: V1AcceptanceResult["etEstablished"]["stateBinding"] =
    declaration !== null &&
    baseCommit !== null &&
    declaration.baseCommit === baseCommit &&
    declarationLoad.changedFromBase === true &&
  ...
  const resultWithoutResultIdentity = {
    contractVersion:
      V1_ACCEPTANCE_CONTRACT_VERSION as V1AcceptanceResult["contractVersion"],
    status,
    reason,
...
export async function evaluateV1Acceptance(
  options: EvaluateV1AcceptanceOptions,
): Promise<V1AcceptanceResult> {
