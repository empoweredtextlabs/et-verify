...
import type { SemanticLimitation } from "../findings/types.js";
import type {
...
function findingIdentity(observation: Cf003ObservationResult): string {
...
function observedFinding(
  ...
  const decisiveFacts: Cf003MismatchDecisiveFact[] = observation.mismatches.map(
    (mismatch, index) => ({
    ...
    limitations: [
      {
        ...
        message:
          "This finding is limited to the normalized claim type and explicit changed-path set supplied to CF-003.",
      },
    ],
...
function notObservedFinding(
  ...
  const decisiveFact: Cf003BoundedComparisonDecisiveFact = {
    ...
    statement:
      "No admitted mismatch was observed between the normalized scope claim and the supplied actual changed-path set.",
  };
  ...
  return {
    ...
    consequence: "ACCEPT",
    summary:
      "No admitted scope mismatch was observed for the normalized claim type and supplied changed-path set.",
    ...
    limitations: [
      {
        code: "bounded_cf003_supplied_path_scope",
        message:
          "This result is limited to the supplied changed-path set and normalized claim type; it does not establish code correctness, test success, or behavioral safety.",
      },
    ],
    prohibitedConclusions: [
...
function indeterminateLimitations(
  observation: Cf003ObservationResult,
): SemanticLimitation[] {
...
function indeterminateFinding(
  ...
  const limitationCodes = observation.limitations.map(
    (limitation) => limitation.code,
  );
  const decisiveFact: Cf003IndeterminateDecisiveFact = {
    ...
    statement:
      "The scope claim could not be compared safely because required input was absent, malformed, or unsupported.",
  };
  ...
  return {
    ...
    summary:
      "Claimed change scope could not be compared safely from the supplied inputs.",
    decisiveFacts: [decisiveFact],
    citations: [...observation.citations],
...
export function serializeClaimedScopeFinding(
  finding: Cf003SemanticFinding,
): string {
  return JSON.stringify(canonicalize(finding));
}

export function adjudicateClaimedScope(
  observation: Cf003ObservationResult,
): Cf003SemanticFinding {
