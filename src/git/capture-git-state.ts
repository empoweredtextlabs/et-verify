...
import type {
  GitReadonlyRunResult,
  GitReadonlyRunnerOptions,
} from "./run-git-readonly.js";
import {
  GIT_STATE_ADAPTER_VERSION,
  type GitAcceptanceTargetKind,
  type GitCaptureStatus,
  type GitEvidenceIssue,
  type GitStateCaptureRequest,
  type GitStateCaptureResult,
} from "./types.js";

const ACCEPTANCE_TARGET_KINDS = new Set<GitAcceptanceTargetKind>([
  "COMMIT",
  "TREE",
  "MERGE_RESULT",
  "FINAL_NET_PR_PATCH",
...
const REPOSITORY_CONFIGURATION_LIMITATION: GitEvidenceIssue = {
  code: "repository_local_git_configuration_honored",
  message:
    "Repository-local Git configuration remains readable for repository semantics; executable diff, textconv, and fsmonitor surfaces used by admitted commands were explicitly disabled.",
};
const FINAL_TREE_LIMITATION: GitEvidenceIssue = {
  code: "final_repository_tree_not_captured",
  message:
    "The complete final net PR patch was captured, but the complete final repository tree was not captured.",
};
const REPOSITORY_OWNERSHIP_UNTRUSTED: GitEvidenceIssue = {
  code: "git_repository_ownership_untrusted",
  message:
    "Git refused to inspect this repository because the current process does not trust its ownership. Verification did not bypass Git's ownership protection.",
};

type UnknownRecord = Record<string, unknown>;

export interface CaptureGitStateTestOverrides {
  readonlyRunnerOptions?: GitReadonlyRunnerOptions;
  now?: () => Date;
}
...
function uniqueIssues(issues: readonly GitEvidenceIssue[]): GitEvidenceIssue[] {
  const byIdentity = new Map<string, GitEvidenceIssue>();
...
function normalizedRequest(request: unknown): {
  ...
  const requestedPath =
    typeof record.repositoryPath === "string" ? record.repositoryPath : "";
  const baseRef = typeof record.baseRef === "string" ? record.baseRef : "";
  const headRef = typeof record.headRef === "string" ? record.headRef : "";
  const acceptanceTargetKind = ACCEPTANCE_TARGET_KINDS.has(
    record.acceptanceTargetKind as GitAcceptanceTargetKind,
  )
    ? (record.acceptanceTargetKind as GitAcceptanceTargetKind)
    : null;
  const errors: GitEvidenceIssue[] = [];
...
function failureDetail(result: GitReadonlyRunResult): string {
  if (result.ok) {
...
function runnerFailureIssue(result: GitReadonlyRunResult): GitEvidenceIssue | null {
...
function isRepositoryOwnershipUntrusted(
  result: GitReadonlyRunResult,
): boolean {
  if (result.ok) {
    return false;
  }
...
function decodeSingleObjectId(result: GitReadonlyRunResult): string | null {
  if (!result.ok) {
    return null;
  }
  try {
...
function decodeSinglePath(result: GitReadonlyRunResult): string | null {
  if (!result.ok) {
    return null;
  }
  try {
...
function parseNulPaths(buffer: Buffer): string[] | null {
...
function parseStatusEntries(buffer: Buffer): string[] | null {
  if (buffer.byteLength === 0) {
    return [];
  }
  if (buffer[buffer.byteLength - 1] !== 0) {
...
async function explicitRepositoryPath(
  requestedPath: string,
): Promise<{ resolvedPath: string | null; error: GitEvidenceIssue | null }> {
  ...
  try {
    ...
    return {
      ...
      error: {
        ...
        message:
          "The supplied path is not an explicit Git working-tree root; parent-directory discovery is not used.",
      },
    };
  }
...
function addCommandFailure(
  errors: GitEvidenceIssue[],
  result: GitReadonlyRunResult,
  issue: GitEvidenceIssue,
): void {
  errors.push(issue);
...
async function captureGitStateInternal(
  captureRequest: GitStateCaptureRequest,
  options: CaptureGitStateTestOverrides,
): Promise<GitStateCaptureResult> {
  ...
  if (root === null || !sameResolvedPath(root, repository.resolvedPath)) {
    if (isRepositoryOwnershipUntrusted(rootResult)) {
      ...
      addCommandFailure(result.errors, rootResult, {
        code: "git_repository_not_a_repository",
        message:
          "The supplied path could not be verified as the exact Git working-tree root.",
      });
    }
    result.errors = uniqueIssues(result.errors);
    return result;
  ...
  if (!patchResult.ok) {
    ...
    try {
      ...
      result.errors.push({
        code: "git_patch_capture_failed",
        message:
          "The exact patch bytes were not valid UTF-8 and could not be represented without rewriting them.",
      });
    }
  }
  ...
  if (statusEntries === null) {
    addCommandFailure(result.errors, statusResult, {
      ...
      message:
        "The informational working-tree status could not be captured completely.",
    });
  } else {
    result.workingTree = {
      inspected: true,
    ...
    if (statusEntries.length > 0) {
      result.limitations.push({
        ...
        message:
          "The working tree contains uncommitted changes; the captured diff compares only the resolved refs and excludes ambient working-tree content.",
      });
    }
  }
  ...
  if (request.acceptanceTargetKind === "COMMIT") {
    ...
    result.limitations.push({
      ...
      message:
        "No merge was synthesized; the explicitly supplied head ref is treated as the intended pre-existing merge-result commit.",
    });
  } else if (
    request.acceptanceTargetKind === "FINAL_NET_PR_PATCH" &&
  ...
  result.captureStatus =
    result.errors.length === 0
      ...
      : decisiveFailure
        ? "FAILED"
        : ("INCOMPLETE" satisfies GitCaptureStatus);
...
export async function captureGitStateWithTestOverrides(
  captureRequest: GitStateCaptureRequest,
  options: CaptureGitStateTestOverrides,
): Promise<GitStateCaptureResult> {
